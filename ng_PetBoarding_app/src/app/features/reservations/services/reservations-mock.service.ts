...
export class ReservationsService {
   ...
   */
  validerPaiement(reservationId: string, amountPaid: number): Observable<boolean> {
    // TODO: Appeler l'API backend pour valider le paiement
    const reservation = this.getReservationById(reservationId);
    
    if (!reservation || reservation.statut !== StatutReservation.CREATED) {
      return of(false);
    }
        ...
        if (success) {
          ...
          if (index !== -1) {
            reservationsActuelles[index] = {
              ...reservationsActuelles[index],
              statut: StatutReservation.VALIDATED,
              paidAt: new Date()
            };
            this.reservations.set([...reservationsActuelles]);
          }
